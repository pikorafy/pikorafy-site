...
// PlayStation Store (PS5 / PS4, Spanish EUR prices). Same toolbar + filter panel as /games.

const PAGE_SIZE = 48;

const SORTS: { key: PsSort; label: string }[] = [
...
function url(qs: URLSearchParams) {
  ...
  return s ? `/playstation?${s}` : "/playstation";
}
...
export async function generateMetadata({ searchParams }: { searchParams: Promise<Params> }): Promise<Metadata> {
  ...
  return {
    title: "PS5 & PS4 Games: PlayStation Store Prices & Deals",
    description:
      "Browse PS5 and PS4 games with current PlayStation Store prices in euros, today's discounts, when each sale ends and what's in PS Plus.",
    alternates: { canonical: "/playstation" },
    ...(filtered ? { robots: { index: false, follow: true } } : {}),
  };
}

export default async function PlayStationPage({ searchParams }: { searchParams: Promise<Params> }) {
  const p = parse(await searchParams);
  const { games, total } = await getPsListing({
  ...
  const chips: { label: string; href: string }[] = [
    ...
    ...p.genreSlugs.map((g) => ({ label: PS_GENRES.find((x) => x.slug === g)!.name, href: without(`genre:${g}`) })),
    ...p.platformKeys.map((k) => ({ label: PS_PLATFORMS.find((x) => x.key === k)!.label, href: without(`platform:${k}`) })),
    ...
    <div className="shell" style={{ paddingTop: 48, paddingBottom: 80 }}>
      <div className="section-hd" style={{ marginBottom: 12 }}>
        <div>
          <div className="eyebrow">
            PlayStation · {total.toLocaleString("en")} {filtered ? (total === 1 ? "match" : "matches") : "games"}
          </div>
          <h1 className="h2" style={{ fontSize: "clamp(30px,5vw,48px)" }}>PS5 &amp; PS4 games. <em>Store prices.</em></h1>
        </div>
      </div>
      <p style={{ color: "var(--text-2)", maxWidth: "70ch", lineHeight: 1.6, margin: "0 0 24px" }}>
        {filtered
          ? `${total.toLocaleString("en")} PlayStation games match, most popular first.`
          : `PS5 and PS4 games with current prices from the Spanish PlayStation Store, including what's in PS Plus. Popular games are checked daily. ` +
      ...
      <div className="listing-toolbar">
        ...
        <form action="/playstation" method="get" role="search" className="listing-search">
          ...
          <input type="search" name="q" defaultValue={p.query} placeholder="Search PlayStation games…" aria-label="Search PlayStation games" />
          {[...filterQuery(p, "q")].map(([k, v]) => <input key={k} type="hidden" name={k} value={v} />)}
          {p.sort !== "popular" && <input type="hidden" name="sort" value={p.sort} />}
        </form>
        <SortSelect basePath="/playstation" params={base.toString()} sort={p.sort} options={SORTS} />
      </div>
      ...
      {chips.length > 0 && (
        <div className="active-filters">
          ...
          <Link href="/playstation" className="clear" rel="nofollow">Clear all</Link>
        </div>
      )}

      {games.length === 0 ? (
        <p style={{ color: "var(--text-2)" }}>
          {filtered ? "No games match these filters. Try removing one." : "No PlayStation games here yet — the catalog is still filling up."}
        </p>
      ) : (
        <div className="cards">
          {games.map((g) => <PsCard key={g.igdb_id} game={g} />)}
        </div>
      )}
...
function PsCard({ game }: { game: PsGame }) {
  ...
  const plusOnly = game.sales_status === "plus_only";
  return (
    <Link href={`/playstation/${game.slug}`} className="card boxart" style={{ textDecoration: "none", color: "inherit" }}>
      <div className="cover">
        <FallbackImg srcs={game.image_wide ? [game.image_wide] : []} last={game.image_square} alt={game.title} loading="lazy"
          style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        {onSale && <div className="disc-tag">-{game.discount_pct}%</div>}
      </div>
      <div className="body">
        <div className="meta">
          <span className="plat">{game.platforms.map((pl) => <span key={pl}>{pl}</span>)}</span>
          {status && <span>{status}</span>}
          {game.plus_tier && <span>{PLUS_TIER_LABEL[game.plus_tier] ?? "PS Plus"}</span>}
        </div>
        ...
        <div className="prices">
          <div>
            ...
            <div className="now">
              {game.is_free ? "Free" : game.price !== null ? money(game.price) : plusOnly ? "PS Plus" : "—"}
            </div>
          </div>
          <div className="stores"><b>PS Store</b></div>
        </div>
      </div>
    </Link>
  );
